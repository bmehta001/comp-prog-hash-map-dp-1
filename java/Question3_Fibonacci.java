 ...
 *
 *  When a recursive algorithm solves the SAME smaller problem multiple
 *  times, we call these "overlapping subproblems." Fibonacci has many:
 *  for example, fib(3) is computed twice when calculating fib(5).
 *
 ...
 *  This is the same bottom-up approach as Approach 3, but with space
 *  optimization -- instead of storing all N+1 values in an array, keep
 *  only the two most recent values (prev, curr).
 *
 *  ===========================================================
...
public class Question3_Fibonacci {
    ...
    // Return one of these strings (case and spacing don't matter):
    //   "O(1)", "O(N)", "O(N^2)", "O(2^N)"
    //
    // Think about:
